...
def plot_color_solution_profile(
    csv_path: str,
    instance_col: str = "test_graph",
    solver_col: str = "solver",
    solution_col: str = "num_colors",
    comparison: str = "relative",
    highlight_best: bool = True,
):
    df = pd.read_csv(csv_path)
    df[solution_col].replace([np.inf, -np.inf], np.nan, inplace=True)

    ax = plot_performance_profile(
        data=df,
        instance_column=instance_col,
        strategy_column=solver_col,
        metric_column=solution_col,
        direction="min",
...
def plot_color_lowerbound_profile(
    csv_path: str,
    instance_col: str = "test_graph",
    solver_col: str = "solver",
    lb_col: str = "lower_bound",
    comparison: str = "relative",
    highlight_best: bool = False,
):
    df = pd.read_csv(csv_path)

    df[lb_col].replace([np.inf, -np.inf], np.nan, inplace=True)

    ax = plot_performance_profile(
        data=df,
        instance_column=instance_col,
        strategy_column=solver_col,
        metric_column=lb_col,
        direction="max",
        comparison=comparison,
...
def plot_solvers():
    csv_file = "evaluation_solvers.csv"
    ax = plot_color_lowerbound_profile(
        csv_file
    )
    fig = ax.figure
    fig.savefig("evaluation_solvers_lower_bound.png", dpi=300)
...
def plot_solvers_and_preprocessing():
    ...
    fig = ax.figure
    fig.savefig("evaluation_solvers_and_preprocessing_lower_bound.png", dpi=300)
    ax = plot_color_solution_profile(csv_file)
    fig = ax.figure
    fig.savefig("evaluation_solvers_and_preprocessing_solution.png", dpi=300)


def plot_heuristics():
    csv_file = "evaluation_heuristics.csv"
    ax = plot_color_solution_profile(csv_file)
    fig = ax.figure
    fig.savefig("evaluation_heuristics.png", dpi=300)
